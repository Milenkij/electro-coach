...
- **Push — только по явной команде пользователя.** Никогда не пушить проактивно (в конце серии коммитов, «для удобства», в рамках многошаговой задачи). Явная команда — это сообщение вида «пуш», «запушь», «push», «выложи на remote».
- **Разрешено:** push feature-веток в `origin` (`git push origin <branch>`, `git push -u origin <branch>`).
- **Запрещено без отдельного явного запроса:** `git push --force*` (любой вариант force/force-with-lease), прямой push в `main`/`master`, push тегов.
- **Merge в `main` — только через PR**, мерджит пользователь сам (push в main триггерит CI/CD деплой бота).
